// Config detail page - Show all simulations for a specific configuration
...
async function loadConfigDetails() {
    ...
    try {
        ...
        // Load simulations.json to find all simulations for this config
        const simsResponse = await fetch(`${RESULTS_PATH}/simulations.json`);
        if (!simsResponse.ok) {
        ...
        simulations = (await Promise.all(simulationPromises)).filter(s => s !== null);

        if (simulations.length === 0) {
            throw new Error('No valid simulations found for this configuration');
        }

        // Initialize visibility state (all visible by default)
        simulations.forEach(({ id }) => {
...
function renderConfigDetails() {
    document.getElementById('configTitle').textContent = `Configuration: ${configId}`;
    ...
    const configEntries = Object.entries(configData).map(([key, value]) => {
        ...
        return `<div class="row mb-2">
            ...
            <div class="col-md-9">${displayValue}</div>
        </div>`;
    }).join('');
    ...
    details.innerHTML = `
        ...
        <div class="alert alert-info">
            <strong>${simulations.length}</strong> simulations with this configuration
        </div>
    `;
}
...
function renderSimulationsTable() {
    ...
    tbody.innerHTML = simulations.map(({ id, simData }) => `
        <tr>
...
function renderRegressionPlot() {
    ...
    // Create datasets - one per simulation
...
// Load config details when page loads
document.addEventListener('DOMContentLoaded', loadConfigDetails);